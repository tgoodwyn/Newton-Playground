...
/**
 ...
 A simpler alternative to a JPanel with a CardLayout. The AWT CardLayout layout
 manager can be inconvenient to use because the special "stack of cards"
 operations it supports require a cast to use. For example to show the card
 named "myCard" given a JPanel with a CardLayout one would write:  <pre>
 ...
 myChild.getName() yeilds "MyChildName"
 </pre> As with CardLayout, the first child added to a CardPanel is made visible
 and there's only one child visible at a time. The <code>showCard</code> method
 accepts either a child's name or the child itself:
 <pre>
...
/**
 Set the childs name (if non-null) and and make it visible iff it's the only
 CardPanel child.
...
/**
 Creates a CardPanel. Children, called "cards" in this API, should be added with
 add(). The first child we be made visible, subsequent children will be hidden.
 To show a card, use one of the show*Card methods.
 */
public CardPanel() {
    super(new Layout());
}
...
/**
 Return the index of the first (and one would hope - only) visible child. If a
 visible child can't be found, perhaps the caller has inexlicably hidden all of
 the children, then return -1.
 */
...
/**
 Hide the currently visible child "card" and show the specified card. If the
 specified card isn't a child of the CardPanel then we add it here.
 */
...
/**
 Show the card that was added to this CardPanel after the currently visible
 card. If the currently visible card was added last, then show the first card.
 */
...
/**
 Show the card that was added to this CardPanel before the currently visible
 card. If the currently visible card was added first, then show the last card.
 */
